...
import { Heart, ShoppingCart, Eye, Sparkles, Leaf } from 'lucide-react';
    ...
    useEffect(() => {
        const fetchPersonalCareAndWishlist = async () => {
            try {
                ...
                console.error("Personal Care Fetch Error:", error);
                setLoading(false);
            }
        };
        fetchPersonalCareAndWishlist();
    }, [dispatch, userInfo]);
        ...
        <div className="min-h-screen pt-28 bg-[#FDFBF7] py-20 px-6 lg:px-20 text-[#1A1A1A]">
            ...
            <div className="max-w-7xl mx-auto mb-20">
                <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.4em] text-slate-400 mb-6">
                    <Leaf size={12} /> Advanced Botanicals & Apothecary
                </div>
                <h1 className="text-4xl md:text-6xl font-serif font-medium leading-[0.9]">
                    Veloura <span className="italic text-slate-400">Care</span>
                </h1>
            </div>
                ...
                {products.length === 0 ? (
                    <div className="col-span-full text-center py-24">
                        <p className="text-slate-400 font-serif text-lg italic">
                            No self-care solutions in stock yet! Check back soon.
                        </p>
                    </div>
                ) : (
                    products.map((p) => {
