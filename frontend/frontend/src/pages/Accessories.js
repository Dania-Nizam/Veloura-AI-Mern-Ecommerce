...
import { Heart, ShoppingCart, Eye, Sparkles, Gem } from 'lucide-react';
            ...
            try {
                ...
                const filteredAcc = data.filter(
                    (p) => p.category && (
                        ...
                        p.category.trim().toLowerCase().includes('accessory')
                    )
                );
                setProducts(filteredAcc);
        ...
        <div className="min-h-screen pt-28 bg-[#FDFBF7] py-20 px-6 lg:px-20 text-[#1A1A1A]">
            ...
            <div className="max-w-7xl mx-auto mb-20">
                <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.4em] text-slate-400 mb-6">
                    <Gem size={12} /> Finery & Accoutrements
                </div>
                <h1 className="text-4xl md:text-6xl font-serif font-medium leading-[0.9]">
                    Veloura <span className="italic text-slate-400">Maison</span>
                </h1>
            </div>
                ...
                {products.length === 0 ? (
                    <div className="col-span-full text-center py-24">
                        <p className="text-slate-400 font-serif text-lg italic">
                            Our curated accessories collection is arriving shortly.
                        </p>
                    </div>
                ) : (
                    products.map((p) => {
                        ...
                        const toggleWishlist = async (e) => {
                            ...
                            if (isWishlisted) {
                                try {
                                    ...
                                    await axios.delete(`https://veloura-ai-mern-ecommerce.vercel.app/api/wishlist/${p._id}`, config);
                                } catch (err) {
